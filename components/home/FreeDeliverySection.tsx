...
// components/home/FreeDeliverySection.tsx
// Homepage row for listings with a free delivery override
// (Listing.deliveryFeeOverrideKobo === 0 — same flag behind the "Free
// Delivery" badge on ListingCard). Backed by /api/listings/free-delivery,
// same shape as ZamoraxDirectSection.tsx. Gated on
// subSettings.freeDeliveryEnabled / freeDeliveryCount (admin-managed at
...
// Auto-slide interval — same rhythm as the other homepage carousels
...
export function FreeDeliverySection() {
  ...
  const count = settings.freeDeliveryCount || 8

  useEffect(() => {
    if (!settings.freeDeliveryEnabled) { setLoading(false); return }

    fetch(`/api/listings/free-delivery?limit=${count}`)
      .then(r => r.json())
      .then((data: { listings?: Listing[] }) => setListings(data.listings ?? []))
      .catch(() => setListings([]))
      .finally(() => setLoading(false))
  }, [settings.freeDeliveryEnabled, count])
      ...
      <div className="flex items-start justify-between mb-4 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-bold text-foreground truncate">Free Delivery</h2>
            <p className="text-xs text-muted-foreground">No delivery fee on these listings</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link href="/free-delivery" className="text-xs text-primary font-medium flex items-center gap-0.5 whitespace-nowrap">
